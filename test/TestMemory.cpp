...
TEST(TestMemory, TestSharedPtr)
{
    ...
    }
    {
        ...
        EXPECT("std::shared_ptr<polymorphic>.pure",
            eightser::memory::pure(s_a) == eightser::memory::pure(s_b) &&
            eightser::memory::pure(s_b) == eightser::memory::pure(s_c) &&
            eightser::memory::pure(s_c) == eightser::memory::pure(s_d));
...
#include <Eightser/BuiltIn/weak_ptr.hpp>
